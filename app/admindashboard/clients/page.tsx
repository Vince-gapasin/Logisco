                      ...
                      >
                        ...
                        <td className="p-2 border-r border-slate-200">
                          <input
                            ...
                            className={`w-full bg-transparent border rounded px-1.5 py-1 focus:ring-0 focus:outline-none ${
                              errName
                                ? "border-red-500 bg-red-50/20"
                                : "border-slate-200"
                            }`}
                        ...
                        <td className="p-2 border-r border-slate-200">
                          <input
                            ...
                            className={`w-full bg-transparent border rounded px-1.5 py-1 focus:ring-0 focus:outline-none placeholder:text-slate-400 ${
                              errNum
                                ? "border-red-500 bg-red-50/20"
                                : "border-slate-200"
                            }`}
              ...
              <div>
                ...
                <div
                  className={`relative w-full ${
                    isContractDropdownOpen ? "z-70" : "z-10"
                  }`}
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <td colSpan={4} className="py-12 sm:py-16 text-center">
                  <div className="flex flex-col items-center justify-center px-4">
                    ...
                    <p className="text-slate-600 font-normal text-xs mt-1 max-w-sm">
                      Data for {activeTab} will appear here once you add new records.
                    </p>
                  </div>
                </td>
              </tr>
      ...
      <div className="p-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs font-normal text-slate-700 bg-white">
        <span>
          ...
          {Math.min(endIndex, currentData.length)} of {currentData.length} entries
        </span>
        <div className="flex items-center gap-2">
          <button
...
export default function ClientsPage() {
  ...
  const fetchClientsAndPartners = useCallback(async () => {
    try {
      ...
      const partnerRes = await apiFetch<PartnersResponse>("/api/partners");
  ...
  const handleClientSubmit = async (newRecord: ClientRecord) => {
    try {
      if (editingRecord) {
        ...
        // Note: Make sure to implement the PATCH /api/clients/[id] backend endpoint
  ...
  const handlePartnerSubmit = async (newRecord: PartnerRecord) => {
    try {
      ...
      if (editingRecord) {
        ...
        // Note: Make sure to implement the PATCH /api/partners/[id] backend endpoint
        await apiFetch(`/api/partners/${newRecord.id}`, {
          method: "PATCH",
          body: JSON.stringify(payload),
        });
        setToastMessage("Changes saved successfully.");
